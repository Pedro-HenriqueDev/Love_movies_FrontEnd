...
<div class="w-72 card-height bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700" :class="[erased? 'dark:bg-gray-700': 'dark:bg-gray-800']">
    
    <div class="h-3/4" v-if="!loading">
        <div class="h-full">
            <router-link  :to="{name: 'Movie', params: {id: movie.id}}">
                    <div>
                        <img class=" rounded-t-lg" :class="[erased? 'filter grayscale': '']" :src="imgLink(movie.poster_path)" alt="" />
                    </div>
            </router-link>
        </div>
        <div class="p-5">
