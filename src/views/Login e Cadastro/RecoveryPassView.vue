...
<div class="form mt-3 w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow-md sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
    ...
    <div class="space-y-6" action="#">
        ...
        <button @click="sendEmail()" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            <div v-if="!loading">
                Send Email
            </div>
...
export default {
    ...
    methods: {
        sendEmail() {
            ...
            if(this.email == "") {
                this.loading = false
                this.showMessage(true, true, "Email field is required")
                return;
            }
            axios.post(process.env.VUE_APP_URL_APIUSER + 'forgotpassword',{email: this.email}).then((res) => {
                this.loading = false
                this.showMessage(true, false, res.data)
                console.log(res)
            }).catch((err) => {
                this.loading = false
                this.showMessage(true, true, err.response.data.status)
                console.log(err)
            })
        },
        watchClose(res) {
