...
export default {
  ...
  created() {
    ...
    this.$watch(
      ...
      (toSearch, previousSeach) => {
        if(toSearch != previousSeach) {
          this.page = this.$route.params.page
  ...
  methods: {
    moviesLoved() {
      ...
      axios.get(process.env.VUE_APP_URL_APIUSER + "relations/movies", headers).then(res => {
        this.listmoviesLoved = res.data.data
      }).catch(err => {
        console.log(err)
      })
    },
    ...
    searchMovies() {
      ...
      axios.get(process.env.VUE_APP_URL_SEARCH + "movie" + process.env.VUE_APP_KEY + "&language=pt-BR" + "&page=" + this.page + "&query=" + this.textSearch()).then(res => {
          this.loading = false
